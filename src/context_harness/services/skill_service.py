...
"""
    ...
    def _write_scaffold_skill_html(
        ...
        """Write skill.html - individual skill page with file explorer."""
        display_name = repo_name.split("/")[-1] if "/" in repo_name else repo_name
        # Use regular string to avoid f-string conflicts with JS template literals
        content = """\
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Skill Details - __DISPLAY_NAME__</title>
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .skill-header { background: white; border-radius: 8px; padding: 2rem; margin-bottom: 2rem; box-shadow: 0 2px 8px rgba(0,0,0,0.1); }
        .skill-header h1 { color: #667eea; margin-bottom: 0.5rem; }
        .skill-header .meta { color: #666; margin-bottom: 1rem; }
        .skill-header .version { display: inline-block; background: #e8f5e9; color: #2e7d32; padding: 0.3rem 0.7rem; border-radius: 4px; }
        .skill-header .author { margin-left: 1rem; }
        .install-box { background: #f8f9fa; border: 1px solid #ddd; border-radius: 8px; padding: 1rem; margin-bottom: 1rem; }
        .install-box code { display: block; background: #2d2d2d; color: #f8f8f2; padding: 1rem; border-radius: 4px; overflow-x: auto; }
        .tabs { display: flex; gap: 0; margin-bottom: 1rem; }
        .tab { padding: 0.75rem 1.5rem; background: #e0e0e0; border: none; cursor: pointer; font-size: 1rem; }
        ...
        .files li { padding: 0.5rem; border-bottom: 1px solid #eee; }
        .files li:hover { background: #f5f5f5; }
        .files a { color: #667eea; text-decoration: none; }
        .markdown { line-height: 1.8; }
        .markdown h1, .markdown h2, .markdown h3 { margin-top: 1.5rem; margin-bottom: 0.5rem; }
        .markdown code { background: #f5f5f5; padding: 0.2rem 0.4rem; border-radius: 4px; }
        .markdown pre { background: #2d2d2d; color: #f8f8f2; padding: 1rem; border-radius: 8px; overflow-x: auto; }
        .markdown pre code { background: none; padding: 0; }
        .copy-btn { float: right; padding: 0.3rem 0.6rem; background: #667eea; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 0.85rem; }
...
<body>
    <div class="container">
        <a href="index.html" class="back-link">&larr; Back to skills</a>

        <div id="skill-header" class="skill-header">
            <h1 id="skill-name">Loading...</h1>
            <div class="meta">
                <span id="skill-version" class="version">v0.1.0</span>
                <span id="skill-author" class="author"></span>
            </div>
        </div>

        <div class="install-box">
            <strong>Install:</strong>
            <code id="install-cmd">ch skill install skill-name --registry http://localhost:8080</code>
            <button class="copy-btn" onclick="copyInstall()">Copy</button>
        </div>

        <div class="tabs">
            <button class="tab active" onclick="showTab('readme')">README</button>
            <button class="tab" onclick="showTab('files')">Files</button>
        </div>

        <div id="content" class="content">
            <div id="readme" class="markdown"></div>
    ...
    <script>
        ...
        async function loadSkill() {
            if (!skillName) {
                document.getElementById('content').innerHTML = '<p class="error">No skill specified</p>';
                return;
            }
            ...
            try {
                ...
                if (!skill) {
                    document.getElementById('content').innerHTML = '<p class="error">Skill not found</p>';
                    return;
                }

                document.getElementById('skill-name').textContent = skill.name;
                document.getElementById('skill-version').textContent = 'v' + (skill.version || '0.1.0');
                document.getElementById('skill-author').textContent = skill.author || '';
                document.getElementById('install-cmd').textContent = 'ch skill install ' + skill.name + ' --registry ' + window.location.origin;

                // Load SKILL.md
                const mdResponse = await fetch('/skill/' + skillName + '/SKILL.md');
                if (mdResponse.ok) {
                    let md = await mdResponse.text();
                    // Remove frontmatter
                    md = md.replace(/^---[\\s\\S]*?---\\n/, '');
                    document.getElementById('readme').innerHTML = renderMarkdown(md);
                } else {
                    document.getElementById('readme').innerHTML = '<p>No README available</p>';
                }

                // Load file list (simplified - just show known files)
                const files = ['SKILL.md', 'version.txt', 'CHANGELOG.md'];
                const fileList = document.getElementById('file-list');
                for (const file of files) {
                    const li = document.createElement('li');
                    li.innerHTML = '<a href="/skill/' + skillName + '/' + file + '" target="_blank">' + file + '</a>';
                    fileList.appendChild(li);
                }
            } catch (e) {
                document.getElementById('content').innerHTML = '<p class="error">Failed to load skill details</p>';
            }
        }
        ...
        function showTab(tab) {
            ...
            document.getElementById('readme').style.display = tab === 'readme' ? 'block' : 'none';
