...
When status is `recommended`, generate skeleton following the **skill-creator standard**.

**IMPORTANT**: The skeleton must follow the format defined in `.opencode/skill/skill-creator/SKILL.md`.
Key principles from skill-creator:

1. **Conciseness**: Keep SKILL.md under 500 lines. Only add context Claude doesn't already have.
2. **Progressive Disclosure**: Use three levels - metadata (always), SKILL.md body (on trigger), bundled resources (as needed)
3. **Context7 Integration**: Include placeholder for domain research
4. **Structural Flexibility**: Suggest appropriate pattern (Workflow/Task/Reference/Capabilities)

```markdown
---
name: {skill_name}
description: {Comprehensive 2-4 sentence description covering: (1) What this skill does, (2) Specific triggers - when to use it, (3) What problems it solves. Include all "when to use" information here since the body only loads AFTER triggering.}
---

# {Skill Title}

## CRITICAL: Context7 Research Required

**Before implementing this skill, use Context7 to research the domain:**

```
1. context7_resolve-library-id "{primary_technology}"
2. context7_get-library-docs {library_id} --topic "{relevant_topic}"
3. Use documentation to ensure accuracy in API references and best practices
```

**Technologies to research**: {list technologies from patterns_found}

## Overview

{1-2 concise sentences explaining what this skill enables, based on patterns found}

## Structure Guide

Choose the structure that best fits this skill's purpose:

- **Workflow-Based**: For sequential multi-step processes → Use numbered steps
- **Task-Based**: For tool/capability collections → Use task sections
- **Reference/Guidelines**: For standards and conventions → Use reference sections
- **Capabilities-Based**: For integrated systems → Use capability sections

See `.opencode/skill/skill-creator/SKILL.md` for detailed patterns and examples.

## Implementation

{Brief description of the core workflow or capability based on patterns found}

### Key Patterns Found

{For each HIGH-VALUE pattern, briefly document - keep this concise, move details to references/}

- **{pattern_name}**: {one-line description} (`{file_path}`)

## Resources

### scripts/
{List suggested automation scripts with one-line purposes}

### references/
{List reference documents - move detailed patterns, schemas, examples here to keep SKILL.md lean}

### assets/
{List templates, boilerplate, or other files used in output - if applicable}
```

**Note**: Do NOT include in skeleton (keep in baseline report only):
- Evidence tables with frequency counts
- Refinement priority scores  
- Pattern code snippets (move to references/)
- Footer about skeleton generation
...
- **Put ALL triggers in the description frontmatter** - body only loads after triggering

### Skill-Creator Alignment
- Follow the skill-creator standard in `.opencode/skill/skill-creator/SKILL.md`
- **Conciseness**: Only add what Claude doesn't already know
- **Progressive Disclosure**: Keep skeleton lean, suggest references for details
- **Context7 First**: Always include Context7 research placeholder
- **No Debug Info in Skeleton**: Keep evidence/scores in your JSON output only
